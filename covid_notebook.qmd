...
```{r sélection jour de la semaine}
...
retailw_ts <- ts(feur_datew$retail, end = c(2021, 46), frequency = 365)
plot(retailw_ts)
...
plot(retailw_tsd, col = 1:3, main = "Fréquentation des lieux de vente au détail et loisirs")

#retailw_loess <- tsd(retailw_ts2, method = "loess", s.window = 10)
#plot(retailw_loess, col = 1:4)
```
On y distingue clairement une phase de transition : après le choc initial, la fréquentation entame une progression constante qui franchit la ligne du zéro durant l'année 2021. La stabilisation finale en haut de graphique confirme que les comportements se sont fixés sur un nouveau palier de référence, supérieur à la période pré-pandémie, ce qui suggère un changement structurel et durable des habitudes de fréquentation des lieux de ventes au détails et des loisirs en semaine.

En début de période, ces barres sont très allongées, reflétant l'amplitude extrême des perturbations liées au confinement qui provoque une rupture brutale visible dans les données. À l'inverse, en 2022, les résidus s'écrasent et se resserrent autour de l'axe central. Ce passage de fortes oscillations à des variations très réduites indique que la série a regagné en régularité, les facteurs de perturbation externes ayant perdu de leur intensité. La population a repris ces habitudes après la sortie du confinement.
...
Pour les résidus on voit très clairement l'instabilité extrême de l'année 2020 avec des pics et des creux profonds qui traduisent les réactions immédiates aux annoncesde confinements et déconfinements corrélés avec la réouverture des magasins et loisirs. À mesure que l'on avance vers 2022, l'amplitude de ces barres vertes diminue fortement, ce qui signifie que la fréquentation durant les week-ends est devenue beaucoup plus stable et prévisible avec le temps.
...
Les résidus isolent le "bruit" et les évènements atypiques une fois que la tendance et la saisonnalité ont été lissées. L'amplitude des barres est relativement stable, mais on remarque plusieurs pics verticaux isolés (vers le haut et le bas). Ces anomalies ponctuelles peuvent correspondre à des jours fériés spécifiques ou à des événements météorologiques extrêmes qui ont brusquement modifié la présence des gens à domicile par rapport à une semaine normale.
