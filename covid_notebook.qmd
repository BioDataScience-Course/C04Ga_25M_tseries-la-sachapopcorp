...
```{r}
#retaitw_tsd <- tsd(retailw_ts2, method = "average", order = 4)
#plot(retail_tsd, col = 1:3)

retailw_loess <- tsd(retailw_ts2, method = "loess", s.window = 10)
plot(retailw_loess, col = 1:4)
```

En retirant la composante saisonnière, la courbe deseasoned met en évidence la tendance et le cycle à long terme de vos données. Elle confirme que, malgré les variations de court terme, le mouvement de fond est une remontée constante après le chut burtale dû au confinement de 2020. Le lissage permet de voir que la fréquentation des zones de vente au détail et des loisirs par rapport à la valeur prise en référence semble s'être stabilisée à un niveau légèrement supérieur à celui de l'avant-crise vers la fin 2021, avant de fluctuer plus calmement en 2022.

La partie seasonal isole les variations qui se répètent à intervalles réguliers au cours de l'année. il y a cycle avec des pics et des creux qui reviennent périodiquement. L'amplitude de ces cycles semble relativement stable (entre -20 et +15 environ), ce qui suggère que les comportements d'achat hebdomadaires conservent une certaine saisonnalité propre, indépendamment des chocs économiques majeurs subis par la série globale.
...
```{r}
retailwk_loess <- tsd(retailwk_ts2, method = "loess", s.window = 7)
plot(retailw_loess, col = 1:4)
```

La courbe deseasoned combine la tendance de fond et les résidus après retrait des cycles saisonniers. Elle permet d'observer la trajectoire réelle de l'activité, mettant en évidence encore une fois une chute drastique en 2020 et une reprise progressive mais instable. Le fait que cette courbe dépasse son niveau d'origine en fin de période confirme que la dynamique structurelle des week-ends a fini par surpasser la situation d'avant-crise.

La courbe seasonal isole les variations cycliques répétitives, avec des amplitudes marquées oscillant entre +20 et -30. L'aspect irrégulier de ces cycles montre que la saisonnalité des week-ends n'est pas figée, mais évolue légèrement au fil du temps, reflétant des comportements d'achat ou de loisirs qui, bien que périodiques, restent sensibles au calendrier et aux changements d'habitudes.
