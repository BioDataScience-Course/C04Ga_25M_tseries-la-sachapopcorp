---
title: "Les effets du confinement de la pandémie de Covid19 sur divers paramètres sociaux et économiques : Notebook."
...
Notre but ici est de déterminer l'influence du covid-19 pendant et après la pandémie, sur la fréquentation des lieux du quotidiens
...
L'analyse est faite avec la [SciViews Box 2025](https://www.sciviews.org/software/svbox/) dans [Saturn Cloud](https://saturncloud.io) (Linux Ubuntu 22.04), utilisant le [logiciel R](https://www.r-project.org) (`r R.version.string`) avec le package (pastects) version `r packageVersion("pastecs")` est utilisé pour étudier les séries temporelles.
...
### Analyse de la fréquentation des parcs. (Achille)
...
### Analyse de la fréquentation des stations de transports en commun. (Achille)
...
```{r grocery loess spectrum}
grocery_loess <- tseries(grocery_decomp_loess)
spectrum(grocery_loess[, "seasonal"], spans = c(5, 7))
```

On observe un cycle significatif hebdomadaire, on peut maintenant s'intéréser à la différence entre les jours de la semaine et ceux du week-end, afin de savoir s'il existe une différence dans les fréquentations. Il y a également un cycle à 1 environ, ce qui correspond à une cycle annuel.
...
```{r grocery weekdays spectrum}
spectrum(grocery_tswd2, spans = c(3, 5))
```

La série ne semble pas contenir de cycle significatif une fois les jours de week-ends enlevés (même sur la série filtrée, j'ai essayé cela donne a peu près le même graphique.

```{r grocery loess weekdays}
...
plot(grocery_tswk2,  main = "Série weekend - Fréquentation des épiceries et pharmacies")
```

```{r grocery weekends trendtest}
acf(grocery_tswk2)
...
### Analyse de la la fréquentation des lieux de travail (Oguzhan)
...
Grocery : Il y a un cycle de fréquentation hebdomadaire et une tendance globalement à la hausse. Lorsque l'on regarde séparemment les jours de la semaine et les week-ends, il ne semble pas y avoir de cycle significatif de fréquentation des épiceries et pharamcies. On observe cependant une grosse baisse de fréquentation au moment du lockdown, et une phase assez plate dans les deux cas pendant cette période, puis la fréquentaton pour la semaine comme pour les week-ends remontent petit à petit.

Work : En résumé, l’analyse de la fréquentation des lieux de travail en Wallonie montre un effondrement brutal de plus de 60 % au moment du premier confinement, suivi d’une remontée progressive qui reste toutefois nettement inférieure au niveau de référence pendant toute la période 2020–2021. Les ACF journalière et hebdomadaire confirment une forte dépendance temporelle et une série clairement non stationnaire, avec une saisonnalité surtout hebdomadaire. La décomposition LOESS de la série hebdomadaire met en évidence une tendance croissante mais toujours négative, ainsi qu’une composante saisonnière de faible amplitude par rapport au choc global. L’impact de la COVID-19 sur les déplacements vers les lieux de travail apparaît donc durable, même après la phase de confinement strict, la mobilité professionnelle ne revient pas totalement à la normale, ce qui suggère un maintien important du télétravail et une réduction persistante des déplacements domicile-travail.

Retail : On peut voir une augmentation au long terme à la hausse pour la fréquentation des zones de vente au détail et des loisirs. On peut cependant toujours voir des fluctuations dû aux jours de fermetures. On inintéressant séparément au jours de la semaine et au weekend, il y a une différences dans la série brute mais pas dans la tendance générale ou même dans les résidus.

Residential : Lorsqu'on regarde la fréquentation des zones résidentielles on observe une très forte augmentation provoqué par le confinement généralisé de la population, mais au long terme l'habitude du confinement semble être resté dans les habitudes de la population.
