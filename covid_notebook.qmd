...
```{r}
dates <- seq(as.Date("2020-02-15"), by = "day", length.out = length(parksts))
z <- zoo(parksts, dates)
...
```{r}
dates <- seq(as.Date("2020-02-15"), by = "day", length.out = length(transit_ts))
z <- zoo(transit_ts, dates)
...
```{r}
acf(transit_ts)
spectrum(transit_ts, spans = c(3,5))
plot(trend.test(transit_ts, R = 199))
...
```{r}
transit_tsd <- tsd(transit_ts, method = "loess", s.window = 13, trend = TRUE)
plot(transit_tsd, col = 1:3)

parkstsd <- tsd(transit_ts, method = "average", order = 15, time = 3)
plot(parkstsd, col = 1:3)
parkstsd_ts <- tseries(parkstsd)
spectrum(parkstsd_ts[,"filtered"], spans = c(3,5))
```

Comme l'ont déja démontré nos analyses précédentes, on constate une tendance générale à l'augmentation. De plus, l'analyse montre un creux très marqué au moment du premier confinement.
...
Au vu de l'analyse faite plus haut, l'autocorrélation est très forte et significative et la présence d'une tendance est significative également.

Afin de pouvoir comparer nos valeurs nous allons choisir un intervalle de temps allant de février 2020 à février 2022.\
...
On observe un cycle signifcatif à 50 jours ??? 
(Non pas 50 jours. il s'agit de notre unité de base, l'année (365 jours), DIVISEE par ~50 ce qui nous donne plus ou moins 7 jours.)
