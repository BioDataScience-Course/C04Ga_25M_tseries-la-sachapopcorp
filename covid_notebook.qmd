...
### Fréquentation des stations de transport en commun :
...
Au vu de l'analyse faite plus haut, l'autocorrélation est très forte et significative et la présence d'une tendance est significative également. Afin de pouvoir comparer nos valeurs nous allons choisir un intervalle de temps allant de février 2020 à février 2022.
...
Pour cette analyse, nous allons nous concentrer sur la variation du pourcentage de la fréquentation des zones de vente au détail et des loisirs par rapport à la période prise comme référence. En reprenant les dates entre le 15 février 2020 et le 15 février 2022.

```{r}
feur_date<-sfilter(feur, date >= as.Date("2020-02-15"),  date <= as.Date("2022-02-15"))
...
#retailw_loess <- tsd(retailw_ts2, method = "loess", s.window = 7)
...
```{r}
retailwk_tsd <- tsd(retailwk_ts2, method = "average", order = 4)
plot(retailwk_tsd, col = 1:3)
```
