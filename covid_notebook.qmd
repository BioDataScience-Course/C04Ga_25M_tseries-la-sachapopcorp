...
#retail_tsd_series<- tseries(retail_tsd)
```
...
retailw_loess <- tsd(retailw_ts2, method = "loess", s.window = 5)
plot(retailw_loess, col = 1:4)
```

La série temporelle montre une reprise progressive mais instable après le choc de 2020, avec des rechutes marquées en 2021 avant une stabilisation au-dessus du niveau de référence en 2022. Les outils statistiques (ACF et périodogramme) confirment que cette évolution n'est pas aléatoire mais dictée par une tendance de fond dominante et une forte dépendance entre les semaines successives.

Le modèle de tendance est validé par des tests de diagnostic robustes. L'analyse des résidus et le graphique Quantile-Quantile montrent une distribution équilibrée, ce qui prouve que la trajectoire haussière observée est structurelle et fiable pour décrire la réalité de la fréquentation en semaine.
...
plot(retailwk_ts)
# Aggrégation par moyenne des valeurs par semaine
retailwk_ts2 <- aggregate(retailwk_ts, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(retailwk_ts2)
```
...
```{r}
#retaitwk_tsd <- tsd(retailwk_ts2, method = "average", order = 4)
#plot(retail_tsd, col = 1:3)
