...
feur_datewk[feur_datewk$weekdays == "Saturday" | feur_datewk$weekdays == "Sunday" , 2:7] <- NA 
# La série temporelle est créée (début le 15 février 2020 = 46e jour de l'année 2020)
grocery_ts <- ts(feur_datewk$grocery, start = c(2021, 46), frequency = 365)
plot(grocery_ts)
# Aggrégation par moyenne des valeurs par semaine
grocery_ts2 <- aggregate(grocery_ts, 52, FUN = collapse::fmean, ts.eps = 1/52)
plot(grocery_ts2, main = "Série des jours de la semaine - Fréquentation des épiceries et pharmacies")
...
```
<<<<<<< HEAD
Sur le premier graphique, la série brute, on peut voir que qu'il n'y a pas de différence avec les valeurs de références au début de la série, mais la série est suivit d'une chute brutale de la fréquentation avec -80% au printemps, en raison du premier confinement.
...
Sur le premier graphique on peut voir que qu'il n'y a pas de différence avec les valeurs de références, mais la série est suivit d'une chute brutale de la fréquentation avec -80% au printemps, en raison du premier confinement. Une augmentation des fréquentation à la fin de confinement, et on peut observer une seconde baisse à la fin de l'année 2020 en raison de la recrudescence des cas de covid-19. Puis a nouveau un augmentation avec certaines valeurs extrême.
>>>>>>> eb6daa11044fffad3ace2576db17d0bbc18d6103
...
<<<<<<< HEAD
En faisant une analyse de la tendance générale par la méthode bootstrap, on remarque que notre histogramme se trouve à gauche de la valeur t bootstrappée. Cela signifie que p est inférieur à α, nous rejetons donc H0 et concluons qu’une tendance générale significative au seuil α existe dans la série.
=======
